...
class dataset:
  ...
  def mkVocabs(self,args):
    ...
    self.SUM = data.RawField()
    self.GOLD = data.RawField()
    self.REL.is_target = False 
    self.ENT.is_target = False 
    self.fields=[("src",self.INP),("ent",self.ENT),("nerd",self.NERD),("rel",self.REL),("out",self.OUTP),("gold",self.GOLD),("sum", self.SUM)]
    ...
    # <ENTITY_i> 추가
    for i in range(40):
      new_outp_vocab.append('<ENTITY_' + str(i) + '>')
    ...
    self.NERD.vocab.itos = generics
    for i, x in enumerate(generics):
      self.NERD.vocab.stoi[x] = i
